...
module Lowlevel = struct
  ...
  let x  = field vertex "x" double
  let y  = field vertex "y" double
  let () = seal vertex

let opengks = foreign "gr_opengks" (void @-> returning void)
let closegks = foreign "gr_closegks" (void @-> returning void)
let inqdspsize = foreign "gr_inqdspsize" (ptr double @-> ptr double @-> ptr int @-> ptr int @-> returning void)
module Ws = struct
  ...
  *)
  let open_ = foreign "gr_openws" (int @-> string @-> int @-> returning void)
  let close = foreign "gr_closews" (int @-> returning void)
  let activate = foreign "gr_activatews" (int @-> returning void)
  let deactivate = foreign "gr_deactivatews" (int @-> returning void)
  let clear = foreign "gr_clearws" (void @-> returning void)
  let update = foreign "gr_updatews" (void @-> returning void)
end
...
*)
let polyline = foreign "gr_polyline" (int @-> ptr double @-> ptr double @-> returning void)

let polymarker = foreign "gr_polymarker" (int @-> ptr double @-> ptr double @-> returning void)
let text = foreign "gr_text" (double @-> double @-> string @-> returning void)
let inqtext = foreign "gr_inqtext" (double @-> double @-> string @-> ptr double @-> ptr double @-> returning void)
let fillarea = foreign "gr_fillarea" (int @-> ptr double @-> ptr double @-> returning void)
let cellarray = foreign "gr_cellarray" (double @-> double @-> double @-> double @-> int @-> int @-> int @-> int @-> int @-> int @-> ptr int @-> returning void)
let gdp = foreign "gr_gdp" (int @-> ptr double @-> ptr double @-> int @-> int @-> ptr int @-> returning void)
let spline = foreign "gr_spline" (int @-> ptr double @-> ptr double @-> int @-> int @-> returning void)
let gridit = foreign "gr_gridit" (int @-> ptr double @-> ptr double @-> ptr double @-> int @-> int @-> ptr double @-> ptr double @-> ptr double @-> returning void)
...
let with_ws ?(typ=PNG) plot =
  ...
  try
    Lowlevel.Ws.open_ id ("plot" ^ string_of_int id) (int_of_workstation_type typ);
    plot id;
    Lowlevel.Ws.close id
  with exn -> (try Lowlevel.Ws.close id with _ -> ()); raise exn
